...
;(function() {
   ...
   */
  //函数功能: 以thisArg为上下文, 以args为参数, 执行func函数
   ...
   */
  // 函数功能: 遍历数组,
  // 任何一次iteratee函数执行结果返回false, 遍历都将终止, 返回原数组
   ...
   */
  // 函数功能: 遍历数组
  // 如果predicate函数对数组内每一个元素的执行结果都为真则返回真,否则返回假
   ...
   */
  //函数功能: 遍历并过滤数组
  //判断数组的每一个元素对predicate函数执行时否返回true, 返回真,则将该数组元素纳入结果集,否则不纳入结果集
   ...
   * @returns {boolean} Returns `true` if `target` is found, else `false`.
   */
  // 函数功能: 遍历数组array, 查看array中是否包含这样一个元素, 即让comparator函数返回值为真的函数
  // 如果没有找到让comparator函数返回true的元素, 则返回false
   ...
   */
  // 函数功能: 遍历数组, 并对数组的每一个元素进行iteratee函数操作,函数返回值放入结果数组中, 最后返回结果数组
   ...
   * @returns {Array} Returns `array`.
   */
  //函数功能: 将values数组中的值, 一一push进array数组中, 最后返回新的加长版新数组
  //这里为什么不使用Array.prototype.concat来合并数组呢? 或者为什么不使用Array.prototype.push来给array添加元素呢?
   ...
   */
  // 函数功能: 通过函数返回值的真假来查找元素位置, 找到了,则返回位置索引, 没找到就返回-1
  // fromRight: 是否从右边开始遍历, 如果从右边开始遍历,则让fromIndex++, 否则,让fromIndex--
   ...
   * @returns {number} Returns the index of the matched value, else `-1`.
   */
  // 函数功能: 遍历array, 从fromIndex位置开始, 寻找value在array中出现的位置索引, 存在则直接返回,不存在则返回-1
  // 如果查找的value值是NaN, 则需要启用baseFindIndex来做查找,因为NaN === NaN会返回false
  // ?这里为什么不直接使用Array.prototype.indexOf来查找呢?
